# sovereign-stack v2.1: The Sovereign Blueprint
...
The wizard will guide you through setting up your domain, secrets, and backup paths. For detailed post-install steps (MQTT/Step-CA Fingerprints), see **[INSTALL.md](./INSTALL.md)**.
...
2.  **Archive:** All project files (minus media) are compressed.
3.  **Encrypt:** Archives are secured with AES-256 (PBKDF2) using OpenSSL.
4.  **Push:** Encrypted files are transferred to a workstation via SFTP.
...
At `04:30`, the `monitor_backup.sh` script verifies that a fresh backup exists. If no file is found within the 90-minute window, a **High-Priority Alert** is triggered to indicate a potential failure in the hardware or pipeline.
...
3. Refer to **[RESTORE.md](./RESTORE.md)** for the full procedure.
