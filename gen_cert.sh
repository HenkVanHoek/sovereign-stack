...
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
    ...
    # 2. Collect only the subdomain prefix
    read -p "Enter Subdomain prefix (e.g., 'vault' for vault.$DOMAIN): " PREFIX
    read -p "Enter Duration (e.g., 8760h for 1 year, 87600h for 10 years): " DURATION

    # 3. Construct the Full FQDN and filenames
    # This makes the script truly generic and automated
    ...
    # -it allows for the provisioner password prompt
    docker exec -it step-ca step ca certificate "$FULL_FQDN" "$CRT_FILE" "$KEY_FILE" \
        --provisioner "$PROVISIONER" \
        --not-after="$DURATION"

    # 5. Verify success and handle files
    if [ $? -eq 0 ]; then
