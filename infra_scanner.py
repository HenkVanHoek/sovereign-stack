...
# Import the project-wide version from central version.py
try:
...
def sync_to_netbox(host_info, scan_results):
    ...
    try:
        if scan_results["vms"]:
            ...
            c_type = nb_client.virtualization.clusters.cluster_types.get(
                name="VirtualBox")
            if not c_type:
                c_type = nb_client.virtualization.clusters.cluster_types.create(
                    name="VirtualBox",
                    slug="virtualbox"
                )
            ...
            for vm_name in scan_results["vms"]:
                vm_obj = nb_client.virtualization.virtual_machines.get(
                    name=vm_name,
